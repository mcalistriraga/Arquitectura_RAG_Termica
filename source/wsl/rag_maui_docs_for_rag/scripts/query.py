...
MAX_TEMP = 62.0   # umbral de seguridad (controlado en logger)
...
def main():
    ...
    while True:
        ...
        if check_temp_abort():
            log_step("ABORT_BEFORE_EMBEDDING", selected_mode)
            print("🛑 ABORT EMBEDDING")
            continue
        ...
        if check_temp_abort():
            log_step("ABORT_BEFORE_SEARCH", selected_mode)
            print("🛑 ABORT SEARCH")
            continue
        ...
        if check_temp_abort():
            log_step("ABORT_BEFORE_LLM", selected_mode)
            print("🛑 ABORT LLM")
            continue
